...
def model():
    return lgb.LGBMClassifier(device="gpu", n_estimators=100, importance_type="gain")
...
def plot_feature_selection(selectors):
    ...
    plt.savefig("lightGBM_cv_scores.png")


def save_data_to_csv(df_features):
    df_features.to_csv(f"lightGBM_feature_ranking.csv")
